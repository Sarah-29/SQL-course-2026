...
SELECT ps.ward, count(*) as NumPatients, sum(ps.tariff) as tariff FROM PatientStay ps group by ps.ward
...
SELECT
	...
	,ROW_NUMBER() OVER (PARTITION BY ps.Hospital ORDER BY ps.PatientId) AS PatientByHospitalIndex -- row number and count * works the same way
...
SELECT
	ps.PatientId
	,ps.Tariff
	,ROW_NUMBER() OVER (ORDER BY ps.Tariff DESC) AS PatientRowIndex -- row number will always be unique and will not give the same value to ties
